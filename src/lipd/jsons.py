...
def read_jsonld(datadir):
	...
	try:
		...
		_filename = [os.path.join(datadir, file) for file in os.listdir(datadir) if file.endswith(".jsonld")][0]
		if not _filename:
			_filename = [os.path.join(datadir, file) for file in os.listdir(datadir) if file.endswith(".json")][0]
...
def write_json_to_file(json_data, datadir, filename="metadata"):
	"""
	...
	# Write json to file
	try:
		open("{}.jsonld".format(datadir + "/" + filename), "wb").write(json_bin)
